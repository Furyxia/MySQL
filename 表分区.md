...
这一节将讨论带有主键和唯一键与分区键的关系。控制这种关系的规则可以表示为以下几点：分区表中所有被用于分区表达式中的所有分区键必须是表可能有用的每一个唯一键的一部分。

也就是说，表中的每一个唯一键必须使用表分区表达式中每一列。（这也包括表的主键，因为它被定义为一个唯一键）例如，以下的每一个创建表的语句都是无效的：
...
在每种情况下，都建议表中至少有一个唯一键并没有包含分区表达式中的所有列。

以下的每个语句都是有效的，并且表示可以使前面的无效创建语句变成有效语句的一种方式：
...
这个`CREATE TABLE`语句之所以会失败，是因为col1和col3都被包含在建议的分区键中，但是这些列都不是表中两个唯一键中的一部分。下面展示了一种可以修改该无效表定义语句的方法：
...
这种情况下，col3同时属于两个唯一性索引的键，所以表的创建语句能成功执行。

以下表是没办法分区的，因为分区键中并没有包含两个同时属于两个唯一性索引的键:
...
因为主键也是唯一性索引键，这个限制同样适用于表的主键。例如，以下两个语句都是无效的：
...
在两种情况下，主键并没有包含所有分区表达式中涉及的列。然而，下面的两个语句都是有效的：
...
如果表中没有唯一索引（包括没有主键），那么这个限制不适用，你可以在分区表达式中使用任何列，只要列的类型满足分区类型。

出于同样的原因，你不能再向分区表添加唯一索引，除非这个索引包含了所有表的分区表达式中使用的列。考虑以下创建分区表的语句：
...
可以使用如下`ALTER TABLE`语句向t_no_pk表中添加主键：
...
但是以下的语句是无效的，因为c1是分区键的一部分，但不是提议主键的一部分：
...
因为t_no_pk的分区表达式中仅有c1列，所以尝试仅仅在c2添加一个唯一索引将会失败。然而，你可以添加一个同时使用了c1和c2的唯一索引。

这个规则也适用于已经存在的未分区表，你想对这个表使用`ALTER TABLE ... PARTITION BY`对其进行分区。考虑如下创建np_pk表的语句：
...
以下的`ALTER TABLE`语句将会报错，因为added列并不是表的任何唯一索引：
...
表np_pk的情况，唯一能被用作分区表达式一部分的列是id列；如果你想在这个表的分区表达式使用其他列，必须先修改这个表，或者向主键添加期望的列，或者删除所有的主键。

### 2.与存储引擎有关的分区限制

在MySQL8.0中，分区支持实际上并不是MySQL服务提供的，而是由表的存储引擎或本地分区处理程序提供的。在MySQL8.0，只有InnDB存储引擎提供本地分区处理程序。这意味着分区表不能由使用其他存储引擎的语句创建。

#### 注意：

MySQL客户端的NDB存储引擎也提供本地分区支持，但在MySQL8.0中不支持。

`ALTER TABLE ... OPTIMIZE PARTITION`无法与使用InnoDB分区的表正常的工作。对区域此类表，请使用`ALTER TABLE ... REBUILD PARTITION`和`ALTER TABLE ... ANALYZE PARTITION`。

**升级分区表**

当进行升级时，使用KEY分区的表必须转储和重载。使用InnoDB存储引擎以外存储引擎的分区表无法从MySQL5.7升级到MySQL8.0或者更高级的版本，、；在升级之前，必须使用`ALTER TABLE ... REMOVE PARTITIONING`删除这类表的分区或者将使用`ALTER TABLE ... ENGINE=InnoDB`其存储引擎更改为InnoDB。

### 3.与函数有关的分区限制

分区表达式中只允许有如下显示的MySQL函数：
* ABS()
* CEILING()
* DATEDIFF()
* DAY()
...
在MySQL8.0中，分区修剪支持`TO_DAYS()`，`TO_SECONDS()`，`YEAR()`和`UNIX_TIMESTAMP()`函数。

**`CEILING()和FLOOR()`**

这两个函数只有在传递一个确切的数字类型的参数（例如INT类型或DECIMAL类型）时才返回一个整数。这意味着，例如以下的`CREATE TABLE`语句将失败并报错：

```
mysql> CREATE TABLE t (
    -> c1 FLOAT
    -> )
    -> PARTITION BY LIST(FLOOR(c1)) (
...
**带有WEEK说明符的`EXTRACT()`函数**

`EXTRACT()`函数返回的值（当使用`EXTRACT(WEEK FROM col)`时），取决于default_week_format系统变量的值。因此，当`EXTRACT()`将单位指定为`WEEK`时，不允许将其作为分区函数。
